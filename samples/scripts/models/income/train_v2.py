...
PREDICTOR_PATH = get_model_folder("models", "predictors")
EXPLAINER_PATH = get_model_folder("models", "explainers", f"alibi-{alibi.__version__}")
...
class PredictorWrapper:
    ...
    def save(self, base_path: Path = PREDICTOR_PATH):
        ...
        save_dict(save_path / "model-settings.json", self.get_model_settings())
        save_dict(save_path / "settings.json", {"debug": True})
...
class ExplainerWrapper:
    ...
    def save(self, base_path: Path = EXPLAINER_PATH):
        ...
        self.explainer.save(save_path / "data")
        save_dict(save_path / "model-settings.json", self.get_model_settings())
        save_dict(save_path / "settings.json", {"debug": True})
    ...
    def get_model_settings(self) -> Dict:
        return {
            ...
            "parameters": {
                ...
                "version": "v0.1.0",
                "extra": {
                    "infer_uri": f"http://localhost:8080/v2/models/{self.predictor_name}/infer",
                    **self.get_extra_model_settings()
                }
            }
        }
